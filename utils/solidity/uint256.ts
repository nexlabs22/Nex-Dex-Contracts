...
import { ATTR_TYPE, SOLIDITY_UINT256 } from "../constant"
...
export const UnsignedBigNumber = (
  ...
  base = 10
): UnsignedBigNumberType => {
  const helper = (func: any, args: Array<any>): BigNumber => {
    if (instanceOfUnSignedBigNumber(args[1])) args[1] = args[1].value
  ...
  obj.plus = function (
    bigNumber: UnsignedBigNumberType | number | string,
    base = 10
  ): UnsignedBigNumberType {
