...
import { uint256, isUint256 } from "./solidity"
import BigNumber from "bignumber.js"

export const toBigNumber = (e: EtherBigNumber): int256 => int256(e.toString() as string)
export const toEtherBigNumber = (e: any): EtherBigNumber =>
  ethers.utils.parseEther(int256(e).dividedBy(1e18).toString())
...
export const toWeiBigNumber = (e: any): int256 => int256(e).multipliedBy(1e18)
...
export const compareResult = (contractValue: any, expectedValue: any, diff = 0): boolean => {
  if (int256(contractValue).minus(int256(expectedValue)).abs().gt(diff)) {
    console.log(
      "Not equal! - ",
      int256(contractValue).toPrecision(),
      int256(expectedValue).toPrecision()
    )
    return false
  }
  return true
